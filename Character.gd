...
func _process(delta):
    if !Engine.editor_hint:
        ...
        $Hull.global_position = interp_pos.back()
...
const item_db = {
    ...
    "potion" : {
        ...
        name = "Potion",
        description = "A healing potion. Restores 5 HP.",
        hp = 5,
        usable = true,
        consumable = true
    },
...
onready var logger = get_tree().get_nodes_in_group("Logger")[0]
